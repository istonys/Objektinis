...
void Rusiuoti(deque<Studentas>& grupe) {
    ...
    while (true) {
        ...
        if (tikrinti == 1) {
            deque<Studentas> V_galvociai;
            deque<Studentas> V_vargsiukai;
            auto start = std::chrono::high_resolution_clock::now();
            for (int i = 0; i < grupe.size(); i++) {
            ...
            for (int i = 1; i < Vargsiukai.size(); i++) {
                vargsiukai << setw(15) << Vargsiukai[i].vardas << setw(15) << Vargsiukai[i].pavarde << setw(18) << fixed << setprecision(2) << Vargsiukai[i].vidurkis << setw(17) << Vargsiukai[i].mediana << endl;
            }
            vargsiukai.close();
            ofstream galvociai("galvociai.txt");
            galvociai << setw(15) << "Vardas" << setw(15) << "Pavarde" << setw(35) << "Galutinis vid. / Galutinis med." << endl;
