...
void Vidurkis(list<Studentas>& grupe) {
    for (auto i = grupe.begin(); i != grupe.end(); i++) {
        int suma = 0;
        for (auto it = i->pazymiai.begin(); it != i->pazymiai.end(); it++) {
            suma += *it;
        }
        i->vidurkis = double(0.4 * (suma / i->pazymiai.size()) + 0.6 * i->egzamino_balas);
    }
}
...
void Mediana(list<Studentas>& grupe) {
    for (auto& studentas : grupe) {
        ...
        int dydis = studentas.pazymiai.size();
        int vidurys = dydis / 2;
        ...
        if (dydis % 2 == 0) {
            auto it2 = it;
            it2--;
            studentas.mediana = 0.4 * (double)(*it + *it2) / 2.0 + 0.6 * studentas.egzamino_balas;
        }
        else {
            studentas.mediana = 0.4 * (double)(*it) + 0.6 * studentas.egzamino_balas;
        }
    }
}
...
void Spausdinti(list<Studentas>& grupe) {
    ...
    for (auto it = grupe.begin(); it != grupe.end(); it++) {
        cout << setw(15) << it->vardas << setw(15) << it->pavarde << setw(18) << fixed << setprecision(2) << it->vidurkis << setw(17) << it->mediana << endl;
    }
}
void Generuoti() {
    string vardas;
...
void Rusiuoti(list<Studentas>& grupe) {
    ...
    while (true) {
        ...
        if (tikrinti == 1) {
            list<Studentas> V_galvociai;
            list<Studentas> V_vargsiukai;
            auto start = std::chrono::high_resolution_clock::now();
            for (auto it = grupe.begin(); it != grupe.end(); it++) {
                if (it->vidurkis >= 5) {
                    V_galvociai.push_back(*it);
                }
                else V_vargsiukai.push_back(*it);
            }
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
            ...
            vargsiukai << "--------------------------------------------------------------------------------" << endl;
            for (auto it = V_vargsiukai.begin(); it != V_vargsiukai.end(); it++) {
                vargsiukai << setw(15) << it->vardas << setw(15) << it->pavarde << setw(18) << fixed << setprecision(2) << it->vidurkis << setw(17) << it->mediana << endl;
            }
            vargsiukai.close();
            ofstream galvociai("galvociai.txt");
            galvociai << setw(15) << "Vardas" << setw(15) << "Pavarde" << setw(35) << "Galutinis vid. / Galutinis med." << endl;
            galvociai << "--------------------------------------------------------------------------------" << endl;
            for (auto it = V_galvociai.begin(); it != V_galvociai.end(); it++) {
                galvociai << setw(15) << it->vardas << setw(15) << it->pavarde << setw(18) << fixed << setprecision(2) << it->vidurkis << setw(17) << it->mediana << endl;
            }
            galvociai.close();
            ...
            auto start = std::chrono::high_resolution_clock::now();
            list<Studentas> V_Vargsiukai;
            V_Vargsiukai.resize(1);

            std::remove_copy_if(grupe.begin(), grupe.end(), std::back_inserter(V_Vargsiukai), arVargsiukas);
            ...
            vargsiukai << setw(15) << "Vardas" << setw(15) << "Pavarde" << setw(35) << "Galutinis vid. / Galutinis med." << endl;
            vargsiukai << "--------------------------------------------------------------------------------" << endl;
            auto it = std::next(V_Vargsiukai.begin());
            for (; it != V_Vargsiukai.end(); it++) {
                vargsiukai << setw(15) << it->vardas << setw(15) << it->pavarde << setw(18) << fixed << setprecision(2) << it->vidurkis << setw(17) << it->mediana << endl;
            }
            vargsiukai.close();
            ofstream galvociai("galvociai.txt");
            ...
            galvociai << "--------------------------------------------------------------------------------" << endl;
            for (auto i = grupe.begin(); i != grupe.end(); i++) {
                galvociai << setw(15) << i->vardas << setw(15) << i->pavarde << setw(18) << fixed << setprecision(2) << i->vidurkis << setw(17) << i->mediana << endl;
            }
            galvociai.close();
            auto stopRasyti = std::chrono::high_resolution_clock::now();
            auto durationRasyti = std::chrono::duration_cast<std::chrono::milliseconds>(stopRasyti - startRasyti);
